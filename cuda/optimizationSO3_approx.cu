...
__global__ void meanInTpS2(float *d_p, float *d_q, uint32_t *z, 
    float *mu_karch, int N) 
{
...
__global__ void meanInTpS2(float *d_p, float *d_q, uint32_t *z, 
    float* d_weights, float *mu_karch, int N)
{
  __shared__ float p[3*6];
  // one J per column; BLOCK_SIZE columns; per column first 3 first col of J, 
  // second 3 columns second cols of J 
...
//  dim3 blocks(w/128+(w%128>0?1:0), h/64+(h%64>0?1:0),1);
  ...
  if(d_weights == NULL)
    meanInTpS2<<<blocks,threads>>>(d_p,d_q, d_z, d_mu_karch,N);
  else
    meanInTpS2<<<blocks,threads>>>(d_p,d_q, d_z, d_weights, d_mu_karch,N);
...
__global__ void sufficientStatisticsOnTpS2( float *d_p, float *Rnorths, float
    *d_q, uint32_t *z, int N, float *SSs
  ) //, float *N)
{
  __shared__ float p[3*6];
...
extern void sufficientStatisticsOnTpS2GPU(float *h_p, float *d_p, 
  float *h_Rnorths, float *d_Rnorths, float *d_q, uint32_t *d_z ,int N,
