...
///* 
// */
//__global__ void vMFCostFctAssignment(float *cost, uint32_t* W, 
...
//        float err = xi[0]*mui[k] + xi[1]*mui[k+6] + xi[2]*mui[k+12];
//        if(err_max < err) {
//          err_max = err;
//          k_max = k;
//        }
...
//  if (d_weights == NULL)
//  {
//    vMFCostFctAssignment<<<blocks,threads>>>(d_cost,d_W,d_x,
//        d_z,d_mu,N);
//  }else{
//    vMFCostFctAssignment<<<blocks,threads>>>(d_cost,d_W,d_x,
...
#pragma unroll
      for (uint32_t k=0; k<6*K; ++k) {
        float err = xi[0]*mui[k] + xi[1]*mui[k+K*6] + xi[2]*mui[k+2*K*6];
        if(err_max < err) {
          err_max = err;
          k_max = k;
        }
...
#pragma unroll
      for (uint32_t k=0; k<K*6; ++k) {
        float err = xi[0]*mui[k] + xi[1]*mui[k+K*6] + xi[2]*mui[k+K*2*6];
        if(err_max < err) {
          err_max = err;
          k_max = k;
        }
